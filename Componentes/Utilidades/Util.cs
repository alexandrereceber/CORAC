...
namespace ServerClienteOnline.Utilidades
{
    ...
    public class Pacote_FrameTelas : ITipoPacote
    {
        ...
        {
            Image Tela = CaptureWindow(User32.GetDesktopWindow());

            FrameTelas[0].CopyTela.CopyFromScreen(new Point { X = 0, Y = 0 }, new Point { X = 0, Y = 0 }, new Size { Width = 300, Height = 300 });
            MemoryStream TransformImg = new MemoryStream();
            FrameTelas[0].TelaMonitor.Save(TransformImg, Configuracoes_Gerais.TiposImagems);
            FrameTelas[0].Primary = Convert.ToBase64String(TransformImg.ToArray());
        }
    }
